...
@section('title', __('messages.update_student'))
@section('breadcrumb_current', __('messages.students'))
@section('page_title', __('messages.update_student'))
    ...
    @if (session('successEditMsg'))
        <div class="mb-10 animate-in fade-in slide-in-from-top-4 duration-500">
            <div
                ...
                <p class="text-sm font-bold">{{ session('successEditMsg') }}</p>
            </div>
        </div>
    @endif
    ...
    <div class="max-w-4xl">
        <div class="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div class="px-10 py-8 border-b border-gray-50 bg-gray-50/50 flex justify-between items-center">
                <div>
                    <h4 class="text-lg font-black text-gray-900 tracking-tighter">Update Student</h4>
                    <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-0.5">Modify student
                        credentials and academic info</p>
                </div>
            ...
            <form class="p-10 md:p-12 space-y-8" method="POST" action="{{route('admin.student.update', $student->id)}}">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative group">
                            ...
                            <input type="password" name="password" id="password"
                                value="{{old('password', $student->password)}}" required
                                class="w-full h-14 bg-gray-50/50 border border-gray-100 rounded-2xl {{ __('messages.page_direction') == 'rtl' ? 'pr-12' : 'pl-12' }} px-6 text-sm font-bold text-gray-900 focus:bg-white focus:border-blue-600 focus:ring-4 focus:ring-blue-600/5 transition-all outline-none">
                        </div>
                        @error('password')
                            <p class="text-[10px] text-red-500 font-bold px-1 uppercase tracking-tight">{{$message}}</p>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative group">
                            ...
                            <input type="text" id="class" name="class" value="{{old('class', $student->student->class)}}"
                                required
                                class="w-full h-14 bg-gray-50/50 border border-gray-100 rounded-2xl {{ __('messages.page_direction') == 'rtl' ? 'pr-12' : 'pl-12' }} px-6 text-sm font-bold text-gray-900 focus:bg-white focus:border-blue-600 focus:ring-4 focus:ring-blue-600/5 transition-all outline-none">
                        </div>
                        @error('class')
