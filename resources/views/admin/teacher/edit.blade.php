...
@section('title', __('messages.update_teacher'))
@section('breadcrumb_current', __('messages.teachers'))
@section('page_title', __('messages.update_teacher'))
    ...
    <div class="max-w-4xl">
        <div class="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div class="px-10 py-8 border-b border-gray-50 bg-gray-50/50 flex justify-between items-center">
                <div>
                    <h4 class="text-lg font-black text-gray-900 tracking-tighter">Update Profile</h4>
                    <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-0.5">Modify teacher
                        credentials and professional info</p>
                </div>
            ...
            <form class="p-10 md:p-12 space-y-8" method="POST" action="{{route('admin.teacher.update', $teacher->id)}}">
                @csrf
                @method('PUT')
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative group">
                            ...
                            <input type="password" name="password" id="password"
                                value="{{old('password', $teacher->password)}}" required
                                class="w-full h-14 bg-gray-50/50 border border-gray-100 rounded-2xl {{ __('messages.page_direction') == 'rtl' ? 'pr-12' : 'pl-12' }} px-6 text-sm font-bold text-gray-900 focus:bg-white focus:border-blue-600 focus:ring-4 focus:ring-blue-600/5 transition-all outline-none">
                        </div>
                        @error('password')
                            <p class="text-[10px] text-red-500 font-bold px-1 uppercase tracking-tight">{{$message}}</p>
                    ...
                    <div class="space-y-2 md:col-span-2">
                        ...
                        <div class="relative group">
                            ...
                            <input type="text" id="subject" name="subject"
                                value="{{old('subject', $teacher->teacher->subject)}}" required
                                class="w-full h-14 bg-gray-50/50 border border-gray-100 rounded-2xl {{ __('messages.page_direction') == 'rtl' ? 'pr-12' : 'pl-12' }} px-6 text-sm font-bold text-gray-900 focus:bg-white focus:border-blue-600 focus:ring-4 focus:ring-blue-600/5 transition-all outline-none">
                        </div>
                        @error('subject')
                            <p class="text-[10px] text-red-500 font-bold px-1 uppercase tracking-tight">{{$message}}</p>
