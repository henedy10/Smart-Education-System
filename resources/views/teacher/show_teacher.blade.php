...
<body class="bg-gray-100 min-h-screen">
    <div class="flex flex-col md:flex-row">
        ...
        <div id="sidebar" class="sidebar w-full md:w-64 bg-white shadow-lg p-4 space-y-4">
            ...
            <nav class="space-y-2">
                <a href="{{route('teacher.lessons.show',$teacher)}}" class="block py-2 px-4 rounded hover:bg-blue-100 text-gray-700"><i class="fas fa-chalkboard-teacher mr-2"></i> {{__('messages.lessons')}}</a>
                <a href="{{route('teacher.homeworkAction.show',$teacher)}}" class="block py-2 px-4 rounded hover:bg-green-100 text-gray-700"><i class="fas fa-tasks mr-2"></i> {{__('messages.homeworks')}}</a>
                <a href="{{route('teacher.quizzes.create',$teacher)}}" class="block py-2 px-4 rounded hover:bg-yellow-100 text-gray-700"><i class="fas fa-file-alt mr-2"></i> {{__('messages.quizzes')}}</a>
                <a href="{{route('teacher.quizzes.show',$teacher)}}" class="block py-2 px-4 rounded hover:bg-red-200 text-gray-700"><i class="fas fa-chart-line mr-2"></i> {{__('messages.results')}}</a>
            </nav>
        </div>
        ...
        <div class="flex-1 p-6 ">
            ...
            <div class="grid md:grid-cols-3 gap-4">
                <div class="bg-white shadow rounded-lg p-4">
                    <h3 class="text-lg font-bold text-blue-600 mb-2">{{__('messages.num_lessons')}}</h3>
                    <p class="text-3xl font-semibold text-blue-800">{{$teacher->lessons_count}}</p>
                </div>

                <div class="bg-white shadow rounded-lg p-4">
                    <h3 class="text-lg font-bold text-green-600 mb-2">{{__('messages.num_homeworks')}}</h3>
                    <p class="text-3xl font-semibold text-green-800">{{$teacher->homeworks_count}}</p>
                </div>

                <div class="bg-white shadow rounded-lg p-4">
                    <h3 class="text-lg font-bold text-yellow-600 mb-2">{{__('messages.num_quizzes')}}</h3>
